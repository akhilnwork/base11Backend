...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('admin.venues.update', $venue) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700">Venue Title</label>
                                    <div class="mt-1">
                    <input type="text" name="title" id="title" value="{{ old('title', $venue->title) }}" 
                ...
                <div>
                    <label for="slug" class="block text-sm font-medium text-gray-700">Slug (URL)</label>
                                    <div class="mt-1">
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $venue->slug) }}" 
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="description" id="description" rows="4" 
                              ...
                              placeholder="Enter venue description">{{ old('description', $venue->description) }}</textarea>
                </div>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
                ...
                <div class="mt-1">
                    <select name="gallery_id" id="gallery_id" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md px-3 py-2 @error('gallery_id') border-red-300 @enderror">
                        ...
                        @foreach($galleries as $gallery)
                            <option value="{{ $gallery->id }}" {{ old('gallery_id', $venue->gallery_id) == $gallery->id ? 'selected' : '' }}>
                                {{ $gallery->title }} ({{ count($gallery->gallery_image_ids ?? []) }} images)
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <div id="coverPreview" class="{{ $venue->coverImage ? '' : 'hidden' }} mb-4">
                            <div class="relative inline-block">
                                <img id="selectedCover" 
                                     src="{{ $venue->getCoverImageThumbUrl() ?: $venue->getCoverImageUrl() }}" 
                                     alt="{{ $venue->title }}" 
                        ...
                        <button type="button" id="selectCoverBtn" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            ...
                            {{ $venue->coverImage ? 'Change Cover Image' : 'Select Cover Image' }}
                        </button>
                        
                        <input type="hidden" name="cover_image_id" id="coverImageId" value="{{ old('cover_image_id', $venue->cover_image_id) }}">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <div id="currentSubImages" class="{{ $venue->sub_image_ids && count($venue->sub_image_ids) > 0 ? '' : 'hidden' }} mb-4">
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $venue->meta_title) }}" maxlength="60"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" name="og_title" id="og_title" value="{{ old('og_title', $venue->og_title) }}" maxlength="60"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="meta_description" id="meta_description" rows="3" maxlength="160"
                                      ...
                                      placeholder="SEO description (160 chars max)">{{ old('meta_description', $venue->meta_description) }}</textarea>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="og_description" id="og_description" rows="3" maxlength="160"
                                      ...
                                      placeholder="Social media description">{{ old('og_description', $venue->og_description) }}</textarea>
                        </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-1">
                        <input type="url" name="og_image" id="og_image" value="{{ old('og_image', $venue->og_image) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentSubImages && !currentSubImages.classList.contains('hidden')) {
        // Venue has existing sub images, initialize the array
        @if($venue->sub_image_ids)
            subImageIds = @json($venue->sub_image_ids);
            console.log('Initialized with existing sub images:', subImageIds);
        @endif
    }
    
    // Initialize hidden inputs for existing sub images
    updateSubImagesInputs();
    
    // Handle remove button clicks for current sub images
    document.addEventListener('click', function(e) {
        if (e.target.closest('.remove-sub-image')) {
            ...
            console.log('Removed sub image at index:', imageIndex);
    ...
    window.handleSubImagesSelection = function(media) {
        console.log('=== EDIT ADDITIONAL IMAGES SELECTION START ===');
        ...
        console.log('=== EDIT ADDITIONAL IMAGES SELECTION END ===');
    };
    ...
    function updateSubImagesDisplay(images) {
        ...
        if (images.length > 0) {
            ...
            selectSubImagesBtn.textContent = 'Select New Additional Images';
        }
    }
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        console.log('- Current sub images element:', currentSubImages);
        console.log('- Current sub images visibility:', currentSubImages.classList.contains('hidden'));
