...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                        ...
                        <textarea name="description" id="description" rows="3" maxlength="500"
                                  ...
                                  placeholder="Brief description of the blog post (500 chars max)">{{ old('description', $blog->description) }}</textarea>
                    </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <div id="featuredPreview" class="{{ $blog->featuredImage ? '' : 'hidden' }} mb-4">
                        <div class="relative inline-block">
                            <img id="selectedFeatured" 
                                 src="{{ $blog->getFeaturedImageThumbUrl() ?: $blog->getFeaturedImageUrl() }}" 
                                 alt="{{ $blog->title }}" 
                    ...
                    <button type="button" id="selectFeaturedBtn" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        ...
                        {{ $blog->featuredImage ? 'Change Featured Image' : 'Select Featured Image' }}
                    </button>
                    
                    <input type="hidden" name="featured_image_id" id="featuredImageId" value="{{ old('featured_image_id', $blog->featured_image_id) }}">
                    <input type="hidden" name="remove_featured" id="removeFeaturedFlag" value="0">
            ...
            <div>
                ...
                <div class="mt-1">
                    <div id="currentSubImages" class="{{ $blog->sub_image_ids && count($blog->sub_image_ids) > 0 ? '' : 'hidden' }} mb-4">
                        <p class="text-sm text-gray-600 mb-2">Current Images:</p>
                        <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                            @foreach($blog->getSubImageThumbUrls() as $index => $imageUrl)
                                <div class="relative group" data-image-index="{{ $index }}">
                                    <img src="{{ $imageUrl }}" alt="Blog image" class="w-full h-16 object-cover rounded border">
                                    <button type="button" class="remove-sub-image absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1 hover:bg-red-700 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button type="button" id="selectSubImagesBtn" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        ...
                        Select New Additional Images
                    </button>
                    
                    <div id="subImagesInputs"></div>
                    <p class="mt-2 text-sm text-gray-500">Choose new images to replace current additional images</p>
                </div>
                @error('sub_image_ids')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="content" id="content" rows="15" 
                              class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md px-3 py-2 @error('content') border-red-300 @enderror">{{ old('content', $blog->content) }}</textarea>
                </div>
                @error('content')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $blog->meta_title) }}" maxlength="60"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" name="og_title" id="og_title" value="{{ old('og_title', $blog->og_title) }}" maxlength="60"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="meta_description" id="meta_description" rows="3" maxlength="160"
                                      ...
                                      placeholder="SEO description (160 chars max)">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="og_description" id="og_description" rows="3" maxlength="160"
                                      ...
                                      placeholder="Social media description">{{ old('og_description', $blog->og_description) }}</textarea>
                        </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-1">
                        <input type="url" name="og_image" id="og_image" value="{{ old('og_image', $blog->og_image) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cleaning options panel functionality
    ...
    // Initialize featured image ID if it exists
    if (featuredImageId.value && featuredImageId.value !== '') {
    ...
    // Initialize sub image IDs if they exist
    let subImageIds = [];
    @if($blog->sub_image_ids)
        subImageIds = @json($blog->sub_image_ids);
    @endif
    
    // Function to update hidden inputs
    function updateSubImageInputs() {
        subImagesInputs.innerHTML = '';
        subImageIds.forEach(function(id) {
    ...
    // Function to update current images display
    function updateCurrentSubImagesDisplay() {
        if (subImageIds.length === 0) {
            currentSubImages.classList.add('hidden');
        } else {
            currentSubImages.classList.remove('hidden');
            // The display will be updated via AJAX or page refresh
        }
    }
    
    // Handle remove button clicks for current images
    document.addEventListener('click', function(e) {
        if (e.target.closest('.remove-sub-image')) {
            const imageContainer = e.target.closest('[data-image-index]');
            const imageIndex = parseInt(imageContainer.dataset.imageIndex);
            
            // Remove from array
            subImageIds.splice(imageIndex, 1);
            
            // Remove from DOM
            imageContainer.remove();
            
            // Update hidden inputs
            updateSubImageInputs();
            
            // Update display
            updateCurrentSubImagesDisplay();
            
            console.log('Removed image at index:', imageIndex);
            console.log('Updated subImageIds:', subImageIds);
        }
    });
    ...
    window.handleSubImagesSelection = function(media) {
        ...
        if (Array.isArray(media) && media.length > 0) {
            ...
            media.forEach(function(item) {
                if (!subImageIds.includes(item.id)) {
                    subImageIds.push(item.id);
                }
            });
            ...
            console.log('Added images to existing selection. Total:', subImageIds.length);
        } else {
